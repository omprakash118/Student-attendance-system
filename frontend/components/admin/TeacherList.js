...
async function showTeacherdetails(teacherId){ 
  ...
  <button onclick="deleteTeacherData('${teacher._id}')" class="cursor-pointer rounded-md border-none bg-red-700 w-full px-4 py-2 text-red-100 transition duration-300 hover:bg-red-500 hover:text-red-50 active:scale-95 active:bg-red-900">Delete</button>
  </div>
  </div>
</div>
</div>
...
<div id="delete-teacher" class="fixed inset-0 bg-[#000000cf] bg-opacity-50 hidden  items-center justify-center p-4 z-50">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md">
            <div class="px-6 py-4">
                ...
                <h3 class="text-lg font-medium text-gray-900 text-center mb-2">Delete Teacher</h3>
                <p class="text-gray-500 text-center">Are you sure you want to delete this teacher? This action cannot be undone.</p>
            </div>
...
function deleteTeacherData(teacherId) {
  const modal = document.getElementById("delete-teacher");
  ...
  newConfirmBtn.addEventListener("click", async () => {
    try {
      const res = await fetch(`http://localhost:8000/api/teacher/delete/${teacherId}`, {
        method: 'DELETE',
      });
      ...
      loadteacherList(); // reload the teacher list
    } catch (err) {
      console.error(err);
